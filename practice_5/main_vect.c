...
#include <sys/time.h>
#include <math.h>
#include <xmmintrin.h> // Для использования __m128
...
void swapMatrix(float *source, float *target, int matrixSize) {
    for (int row = 0; row < matrixSize; row++) {
        for (int col = 0; col < matrixSize; col++) {
            target[col * matrixSize + row] = source[row * matrixSize + col];
        }
    }
}

void multiplyMatrices(float *firstMatrix, float *secondMatrix, float *resultMatrix, int matrixSize) {
    float *B_T = createMatrix(matrixSize);
    swapMatrix(secondMatrix, B_T, matrixSize);
    ...
    for (int row = 0; row < matrixSize; row++) {
        for (int col = 0; col < matrixSize; col++) {
            ...
            for (int k = 0; k < matrixSize; k += 4) {
                if (k + 3 < matrixSize) { 
                    __m128 vecA = _mm_loadu_ps(&firstMatrix[row * matrixSize + k]);
                    __m128 vecB = _mm_loadu_ps(&B_T[col * matrixSize + k]);
            ...
            resultMatrix[row * matrixSize + col] = temp[0] + temp[1] + temp[2] + temp[3];

            for (int k = (matrixSize / 4) * 4; k < matrixSize; k++) {
                resultMatrix[row * matrixSize + col] += firstMatrix[row * matrixSize + k] * B_T[col * matrixSize + k];
            }
        }
    }
...
void scaleMatrix(float *A, float scale, float *resultMatrix, int matrixSize) {
    for (int row = 0; row < matrixSize; row++) {
        for (int col = 0; col < matrixSize; col += 4) {
            if (col + 3 < matrixSize) {
                __m128 vecA = _mm_loadu_ps(&A[row * matrixSize + col]);
                __m128 vecScale = _mm_set1_ps(scale);
                __m128 vecRes = _mm_mul_ps(vecA, vecScale);
                _mm_storeu_ps(&resultMatrix[row * matrixSize + col], vecRes);
            }
        }
        for (int col = (matrixSize / 4) * 4; col < matrixSize; col++) {
            resultMatrix[row * matrixSize + col] = A[row * matrixSize + col] * scale;
        }
    }
}

void addMatrices(float *A, float *B, float *resultMatrix, int matrixSize) {
    for (int row = 0; row < matrixSize; row++) {
        for (int col = 0; col < matrixSize; col += 4) {
            if (col + 3 < matrixSize) {
                __m128 vecA = _mm_loadu_ps(&A[row * matrixSize + col]);
                __m128 vecB = _mm_loadu_ps(&B[row * matrixSize + col]);
                __m128 vecRes = _mm_add_ps(vecA, vecB);
                _mm_storeu_ps(&resultMatrix[row * matrixSize + col], vecRes);
            }
        }
        for (int col = (matrixSize / 4) * 4; col < matrixSize; col++) {
            resultMatrix[row * matrixSize + col] = A[row * matrixSize + col] + B[row * matrixSize + col];
        }
    }
}
...
float *createIdentityMatrix(int matrixSize) {
    ...
    for (int row = 0; row < matrixSize; row++) {
        identityMatrix[row * matrixSize + row] = 1.0f;
    }
...
float *computeB(float *inputMatrix, int matrixSize) {
    ...
    scaleMatrix(transposedMatrix, 1.0f / scalingFactor, B, matrixSize);
...
float *invertMatrix(float *inputMatrix, int matrixSize, int maxIterations) {
    ...
    for (int iteration = 1; iteration < maxIterations; iteration++) {
        addMatrices(resultMatrix, currentTermMatrix, resultMatrix, matrixSize);
